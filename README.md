...
```bash
docker run --name go-postgres-ubuntu --network go-postgres-network -p 80:8080 -p 443:8443 -p 22:22 -itd ubuntu:latest
```
...
```bash
docker run -d --network go-postgres-network --name go-postgres-redis -p 6379:6379 redis:latest
```
